...
ISR(RTC_OVF_vect, ISR_NAKED) {
	...
	struct task *new_task = tasks + TID_IDLE;
	// scheduler goes here
...
void rtos_init(void) {
	...
	/* reset the count and fire off an interrupt every 10 RTC ticks */
	RTC.PER = 10;
...
void rtos_new_task(uintptr_t begin) {
	...
	tasks[task_count].status = TST_RUNNABLE;
	tasks[task_count].sp = next_stack - /*?*/0;
